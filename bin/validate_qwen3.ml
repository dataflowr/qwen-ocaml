...
let parse_int_list s key =
  ...
  | Some g ->
    String.split_on_char ',' (Re.Group.get g 1)
...
let load_ref_logits path n =
  ...
  for i = 0 to n - 1 do
    a.(i) <- Int32.float_of_bits (Bytes.get_int32_le b (i * 4))
  done;
  a
...
let () =
  ...
  Printf.printf "loading Qwen3-0.6B...\n%!";
  let m = Qwen.Model.create Qwen.Config.qwen3_0_6b model_path in
